...
# Assign CRS.
crs(r.canopy) <- CRS('+proj=utm +zone=10 +datum=NAD83 +units=m +no_defs')

##
## Bring in older forest data.
...
# Assign CRS.
crs(r.older) <- CRS('+proj=utm +zone=10 +datum=NAD83 +units=m +no_defs')
```
...
calc.canopy.metrics <- function(x) {
  sample_lsm(r.canopy, y=camera.sites.sf, size=x, plot_id=camera.nests, shape='circle', 
             what=canopy.metrics) %>% 
...
```{r eval=FALSE}
saveRDS(diet.diversity.statistics, '../data/interim/diet_diversity_stats.rds')
saveRDS(data, '../data/interim/diet_landscape_data.rds')
```
...
```{r eval=FALSE}
saveRDS(proportion.squirrel.statistics, '../data/interim/proportion_squirrel_stats.rds')
```
...
```{r eval=FALSE}
saveRDS(productivity.landscape.statistics, '../data/interim/productivity_landscape_stats.rds')
```
