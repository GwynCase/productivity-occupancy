...
Ok, so I originally dropped all the onesies (sites with only one year of survey data) from the analysis because they're almost always active (and if they're not active they're a zero and get dropped anyway), and I was concerned there was a potential for bias.
...
Looks like that filtered out one scale at one site. Does it matter if I don't have a full set of scales per site? That is, if some sites only have some scales? Probably not.
