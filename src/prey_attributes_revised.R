...
remains.data <- read_csv('../data/raw/20200923_specimens.csv', guess_max=7000) %>% 
...
remains.data <- remains.data %>% 
  select(site, nest, class, order, family, genus, species, common, size, source) %>% 
...
diet.items <- left_join(diet.items, average.sizes, by='category') %>% 
  ...
  select(site, nest, class, order, family, genus, species, binomial, common, 
         category, size, mass, method, source)